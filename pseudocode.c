...
int main(){
  ...
  while(true){//loops forever
    while(hasline){//loops while robot has line
      ...
      if(numFound != 0){
        location = sum/numFound; // finds middle of white line
      }
      range = maxP -minP;
      error = center - location;
      if(location == 0 && /*left sensor has wall*/){ //code for entering the fourth quad
        hasline = false;
        continue;
      }else if(/*front sensor has object*/ && location != 0){ //if we have a ine but theres a wall
        /*code for stopping the robot and opening the gate goes here*/
      }else if(range>lineWidth && location < 180){ //this is either a T or 4 way junction or a 90 degree left turn (this might catch sharp left turns to)
        /*code for turning 90 degrees left*/
      }else if(range>lineWidth && location >= 180){ //this is a 90 degree right turn
        /*code for turning 90 degrees right*/
      }else if(location == 0){ //lost line, no wall and no gate
